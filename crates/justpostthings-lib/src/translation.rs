...
pub const DEFAULT_TRANSLATION_PROMPT: &str = "Translate the following text from {from} to {to}. \
...
pub const DEFAULT_SHRINK_PROMPT: &str = "Shorten the following text to fit within {max_chars} characters. \
...
pub fn build_prompt(text: &str, from: &str, to: &str, custom_template: Option<&str>) -> String {
    let template = custom_template.unwrap_or(DEFAULT_TRANSLATION_PROMPT);
    template
        .replace("{from}", from)
        .replace("{to}", to)
        .replace("{text}", text)
...
pub fn build_shrink_prompt(text: &str, max_chars: usize, custom_template: Option<&str>) -> String {
    let template = custom_template.unwrap_or(DEFAULT_SHRINK_PROMPT);
    template
